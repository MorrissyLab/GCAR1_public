...
SPECTRA <- paste("primary_biopsy_DS_RA_M/cNMF_5_30_5_ST/cNMF_5_30_5_ST.gene_spectra_score.k_", RANK, ".dt_2_0.txt", sep = "")
GMT_FILE <- "17_10_24_all_gmt_merge.gmt"
GMT_FILE_ALIAS <- "17_10_24_All_GMT_Merge"

GENE_RANK_PLOTTING <- F
COMPUTE_MGS <- F
TOP_GENES_SPECTRA <- F
gPROFILER_RESULTS <- F

# SSGSEA Parameters and Switches ----
GMT_FILE <- "GCAR1_sarc_gene_set_V2.gmt"
GMT_FILE_ALIAS <- "GCAR_V2"
SSGSEA_GCAR_24um <- F
EXPANDING_SSGSEA_GCAR_2um <- F

# Program Analysis Parameters and Switches ----
usage <- paste("primary_biopsy_DS_RA_M/cNMF_5_30_5_ST/cNMF_5_30_5_ST.usages.k_", RANK, ".dt_2_0.consensus.txt", sep = "")
...
PROGRAM_ENRICHMENT <- F
SPATIAL_CONCORDANCE <- F
PROGRAMS_PER_SPOT <- F
PROGRAM_OVERLAP <- F
USAGE_DENSITY <- F
PROGRAM_GENE_EXPRESSION <- F
T_CELL_SELECTION <- F
...
INSIDE_OUTSIDE_T_CELL <- F
INSIDE_OUTSIDE_PROGRAM <- F
INSIDE_OUTSIDE_NICHE <- F
...
# 4.1 Neighbourhood of each pair of programs ----
...
# 4.2 Custom neighbourhood splitting and dot plot ----
...
# 5.1 Inside-Outside T-Cell Gene Expression Analysis ----
...
# 5.2 Inside-Outside Niche Gene Expression Analysis ----
